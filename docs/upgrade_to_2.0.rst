...
been deprecated in favour loading the session state up-front using
read_session_params() and construct the session from it.

Similarly, the session state can be acquired, in the form of a session_params
object, by calling session::session_state().

The session_params object is passed to the session constructor, and will restore
the state from a previous session.

Use read_session_params() and write_session_params() to deserialize and serialize
...
from a session_pack, to cover one of the now-deprecated constructors. Howerver,
to access this conversion, `libtorrent/session_params.hpp` now has to be included.
